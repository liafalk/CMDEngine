        ...
        : size(size), pos(pos), hidden(false), area(size.x*size.y)
{
    ...
    strcpy(sprite2D, std::string(area, ' ').c_str());
...
bool canvas::sprite::translate(int x, int y){
    ...
    if(newPos.y < parent->size.y-1 && newPos.x < parent->size.x-1 && parent->checkValid(this->sprite2D, newPos, this->size)) {
        pos = newPos;
        return true;
    }
    else
...
void canvas::sprite::rotateCW(){
    ...
    strcpy(tempSprite, std::string(area, ' ').c_str());
    ...
    if (parent->checkValid(tempSprite,newPos,newSize)){
...
void canvas::sprite::rotateCCW(){
    ...
    for(int y=0; y<size.y; ++y){
        for(int x=0; x<size.x; ++x){
            tempSprite[x+y*newSize.x] = sprite2D[(size.x-y-1)+x*(size.x)];
        }
    }
        ...
        : size(size), area(size.x*size.y)
{
    ...
    strcpy(canvas2D, std::string(area, ' ').c_str());
}
...
void canvas::setPixel(Vector2 s_pos, char* s_c, Vector2 s_size){
